#\!/bin/bash
# Final Elastic Beanstalk deployment script with HTTPS configuration and correct instance profile

# Configuration
APP_NAME="coffee-shop"
ENV_NAME="coffee-shop-env"
S3_BUCKET="coffee-shop-app-deployment-garcia-rafael-2274088"
INSTANCE_PROFILE="LabInstanceProfile"  # Using the discovered instance profile
...
# Create new environment with the correct instance profile
echo "Creating new environment with instance profile: $INSTANCE_PROFILE"
...
while [ $WAITED -lt $MAX_WAIT ]; do
  ...
  if [ "$STATUS" == "Terminated" ]; then
    echo "Environment creation failed. Checking events..."
  ...
  echo "Status: $STATUS | URL: $ENV_URL"
  ...
  echo "Waiting for environment to be ready (attempt $WAITED of $MAX_WAIT)..."
  sleep 10
  WAITED=$((WAITED+1))
done
...
3. Select the region: $REGION
4. Click on the application named "$APP_NAME"
5. Click on the environment named "$ENV_NAME"
...
When taking screenshots to demonstrate your implementation, include:

1. **HTTPS Access**: Browser showing https://$ENV_URL with the lock icon
2. **Certificate Details**: Click the lock icon and show certificate details showing your domain
3. **Elastic Beanstalk Dashboard**: AWS Console showing environment with:
   - Apache configuration
   - t3.small instance type
   - IMDSv1 enabled setting

These screenshots will provide complete evidence that you've met all the project requirements.
